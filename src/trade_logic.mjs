...
function getTopTargetPets(globalPetData, refPet, targetType = "Neon Value") {
  ...
  const targetPets = globalPetData
    .filter((pet) => {
      ...
      return (
        !isNaN(targetValue) &&
        pet.name !== refPet.name &&
        targetValue <= maxAffordableValue
      );
    })
  ...
  return globalPetData
    .filter((pet) => {
      ...
      const isSimilarValue =
        !isNaN(petValue) && petValue >= lowerBound && petValue <= upperBound;

      // 2. Better Flipping Potential (Higher Weighted Neon Gain)
      const isBetterGain =
        !isNaN(petWeightedGain) && petWeightedGain > refWeightedGain * 1.01; // Must be at least 1% better
 ...
 * NOTE: This function is preserved but is no longer called in generateTradeRecommendations.
 */
function generateAlternativeBundle(
  targetValue,
  refPet,
  interchangeablePets,
  fixedRefCount
  ...
  if (fixedRefCount * refValue >= targetValue * OPPONENT_WIN_MARGIN)
    return null;
  ...
  const ipsExcludingRef = interchangeablePets.filter(
    (p) => p.name !== refPet.name && p["Regular Value"] > 0
  );
  ...
  if (bestIP.name === refPet.name && fixedRefCount === MAX_TRADE_SLOTS)
    return null;
  ...
  if (ipCount > 0) {
    ...
    if (bestIP.name === refPet.name) {
      if (petBundle.length === 0) {
        // Should only happen if fixedRefCount was 0
  ...
  const totalValue = petBundle.reduce(
...
function findProfitableSellBundles(soldPet, targetType, globalPetData) {
  ...
  for (const petCount of TARGET_COUNTS) {
    ...
    const profitablePetsForCount = globalPetData
      .filter((pet) => {
        ...
        if (isNaN(petRegValue) || petRegValue <= 0 || isNaN(petNeonValue))
          return false;
  ...
  for (const bundlePet of globalPetData) {
    ...
    if (futureProfit > saleValue * 0.1) {
      // Only add this if it's not a count we already generated in Phase 1 (4x)
      const isDuplicate = tradeBundles.some(
        (b) => b.petName === bundlePet.name && b.count === finalCount
      );
 ...
 * * 🟢 FIX 2: Removed erroneous pet lookup
 * * 🟢 FIX 3: Removed all DOM manipulation (This function now only RETURNS HTML)
...
export function generateTradeRecommendations(refPet, globalPetData) {
  // Remove all old DOM manipulation and pet lookup logic
  if (!refPet) {
  ...
  html += `<p style="font-size: 1.1em; font-weight: bold; margin-bottom: 20px;">Current Value: ${formatForDisplay(
    "Regular Value",
    refPet["Regular Value"]
  )} RP</p>`;
  ...
  html +=
    '<div class="rec-section"><h3>🔄 Value Swap Trades (Maximize Aging Profit)</h3>';
  html += `<p>Trade **1x ${
    refPet.name
  }** for 1x of a pet with a similar regular value (±${(
    INTERCHANGEABLE_RANGE * 100
  ).toFixed(0)}%) but a **better Weighted Neon Gain**.</p>`;

  if (valueSwaps.length === 0) {
    html += `<p>No similar-value pets found with significantly better flipping potential.</p>`;
  } else {
    html += `<p>Your Current Weighted Neon Gain: <strong>${formatForDisplay(
      "Weighted Gain",
      refPet["Weighted Neon Gain"]
    )}</strong></p>`;
    ...
    const sortedWNGs = Object.keys(groupedSwaps).sort(
    ...
    sortedWNGs.forEach((wng) => {
      ...
      html += `
              <div class="trade-box" style="background-color: #f0fff0;">
                  <h3>Weighted Neon Gain: ${wng} (+${formatForDisplay(
        "Weighted Gain",
        gainDifference
      )} vs ${refPet.name})</h3>
  ...
  html +=
    '<div class="rec-section"><h3>💰 Profitable Selling Trades (Maximize Value Received for Aging)</h3>';
  ...
  const neonSellBundles = findProfitableSellBundles(
    refPet,
    "Neon",
    globalPetData
  );
  const megaSellBundles = findProfitableSellBundles(
    refPet,
    "Mega",
    globalPetData
  );
  ...
  if (neonSellBundles.length > 0) {
    ...
    html += `<div class="trade-box" style="background-color: #e6ffe6;"><h4>Sell 1x Neon ${
      refPet.name
    } (Value: ${formatForDisplay("Value", saleValue)} RP)</h4>`;
    // --- TARGET VALUE UPDATED FOR 2% ADVANTAGE ---
    html += `<p>Target: Receive a bundle worth up to ${formatForDisplay(
      "Value",
      saleValue / OPPONENT_SELL_ADVANTAGE
    )} RP.</p>`;
    ...
    const groupedBundles = neonSellBundles.reduce((acc, bundle) => {
      ...
      const key = `${bundle.futureProfit.toFixed(2)}~${bundle.agedNeonCount}~${
        bundle.count
      }~${bundle.tradesToComplete}`;
      if (!acc[key]) {
        acc[key] = {
          bundles: [],
    ...
    const sortedGroups = Object.values(groupedBundles).sort((a, b) => {
      const countOrder =
        [1, 2, 4].indexOf(a.count) - [1, 2, 4].indexOf(b.count);
      if (countOrder !== 0) {
        return countOrder;
      }
      return b.futureProfit - a.futureProfit; // Otherwise sort by profit
    ...
    sortedGroups.forEach((group) => {
      const gainPercentage = (
        (group.futureProfit / (group.saleValue * group.tradesToComplete)) *
        100
      ).toFixed(0);
      ...
      html += `<p style="font-weight: bold; color: #28a745; font-size: 1.1em;">
              → Future Profit Potential: ${formatForDisplay(
                "Value",
                group.futureProfit
              )} RP 
              <span style="font-size: 0.9em;">[${
                group.tradesToComplete
              } ${tradeText} $\\to$ 1 Neon pet | ${gainPercentage}% return]</span>
          </p>`;
      ...
      group.bundles.forEach((bundle) => {
        html += `<p style="margin-left: 20px; margin-bottom: 5px; font-size: 0.95em;">
                  - **${bundle.count}x ${
          bundle.petName
        }** (Bundle Value: ${formatForDisplay("Value", bundle.bundleValue)} RP)
              </p>`;
      });
  ...
  if (megaSellBundles.length > 0) {
    ...
    html += `<div class="trade-box" style="background-color: #fff0e6;"><h4>Sell 1x Mega ${
      refPet.name
    } (Value: ${formatForDisplay("Value", saleValue)} RP)</h4>`;
    // --- TARGET VALUE UPDATED FOR 2% ADVANTAGE ---
    html += `<p>Target: Receive a bundle worth up to ${formatForDisplay(
    ...
    sortedGroups.forEach((group) => {
      ...
      html += `<p style="font-weight: bold; color: #28a745; font-size: 1.1em;">
        ...
        group.agedNeonCount
      } Neon pets | ${gainPercentage}% return]</span>
          </p>`;
  ...
  if (tradesGenerated === 0) {
    ...
    // Return the HTML string for index.mjs to use.
    return html;
  }
}
