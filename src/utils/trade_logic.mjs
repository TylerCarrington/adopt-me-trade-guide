...
export function generateTradeRecommendations(refPet, globalPetData) {
    ...
    // --- Inventory Controls ---
    // Use localStorage key 'petInventory' as a JSON object: { [petName]: { regular, neon, mega } }
    // Get inventory from localStorage
    let inventory = { regular: 0, neon: 0, mega: 0 };
    try {
        const inv = JSON.parse(localStorage.getItem('petInventory') || '{}');
        inventory = inv[refPet.name] || inventory;
    } catch {}

    // Inventory controls HTML (one line)
    html += `<h2>Recommendations for ${refPet.name}</h2>`;
    html += `<div id="inventory-controls" style="display:flex;align-items:center;gap:18px;margin-bottom:16px;">
        <span style="font-weight:500;">Inventory:</span>
        <span>
            <span style="margin-right:4px;">Regular</span>
            <button class="inv-minus" data-type="regular" style="width:24px;" type="button">−</button>
            <span id="inv-regular-count" style="min-width:24px;display:inline-block;text-align:center;">${inventory.regular}</span>
            <button class="inv-plus" data-type="regular" style="width:24px;" type="button">+</button>
        </span>
        <span>
            <span style="margin-right:4px;">Neon</span>
            <button class="inv-minus" data-type="neon" style="width:24px;" type="button">−</button>
            <span id="inv-neon-count" style="min-width:24px;display:inline-block;text-align:center;">${inventory.neon}</span>
            <button class="inv-plus" data-type="neon" style="width:24px;" type="button">+</button>
        </span>
        <span>
            <span style="margin-right:4px;">Mega</span>
            <button class="inv-minus" data-type="mega" style="width:24px;" type="button">−</button>
            <span id="inv-mega-count" style="min-width:24px;display:inline-block;text-align:center;">${inventory.mega}</span>
            <button class="inv-plus" data-type="mega" style="width:24px;" type="button">+</button>
        </span>
    </div>`;
    ...
    recommendationsHtml += '<div class="rec-section"><h3>🔄 Value Swap Trades (Maximize Aging Profit)</h3>';
    recommendationsHtml += `<p>Trade **1x ${refPet.name}** for 1x of a pet with a similar regular value (±${(INTERCHANGEABLE_RANGE * 100).toFixed(0)}%) but a **better Weighted Neon Gain**.</p>`;
    if (valueSwaps.length === 0) {
        recommendationsHtml += `<p>No similar-value pets found with significantly better flipping potential.</p>`;
    } else {
        recommendationsHtml += `<p>Your Current Weighted Neon Gain: <strong>${formatForDisplay('Weighted Gain', refPet['Weighted Neon Gain'])}</strong></p>`;
    ...
    recommendationsHtml += '<div class="rec-section"><h3>💰 Profitable Selling Trades (Maximize Value Received for Aging)</h3>';
    recommendationsHtml += `<p>Sell your aged ${refPet.name} for a bundle of high-profit pets to age up next. We calculate a value that gives the opponent a **2% gain** on the ${refPet.name} value.</p>`;
    ...
    if (neonSellBundles.length > 0) {
        ...
        recommendationsHtml += `<div class="trade-box" style="background-color: #e6ffe6;"><h4>Sell 1x Neon ${refPet.name} (Value: ${formatForDisplay('Value', saleValue)} RP)</h4>`;
        recommendationsHtml += `<p>Target: Receive a bundle worth up to ${formatForDisplay('Value', saleValue / OPPONENT_SELL_ADVANTAGE)} RP.</p>`;
        ...
        const sortedGroups = Object.values(groupedBundles).sort((a, b) => {
            ...
            return b.futureProfit - a.futureProfit;
        });
        sortedGroups.forEach(group => {
            ...
            recommendationsHtml += `<div style="margin-top: 15px; padding: 10px; border: 1px solid #c3e6cb; border-radius: 5px;">`;
            recommendationsHtml += `<p style="font-weight: bold; color: #28a745; font-size: 1.1em;">
            ...
            recommendationsHtml += `<p style="margin-top: 10px;"><strong>Offer (Per Trade):</strong> 1x Neon ${refPet.name}</p>`;
            recommendationsHtml += `<p style="font-weight: bold; margin-top: 5px;">Receive Options (Count: ${group.count}):</p>`;
            group.bundles.forEach(bundle => {
                recommendationsHtml += `<p style="margin-left: 20px; margin-bottom: 5px; font-size: 0.95em;">
    ...
    if (megaSellBundles.length > 0) {
        ...
        recommendationsHtml += `<div class="trade-box" style="background-color: #fff0e6;"><h4>Sell 1x Mega ${refPet.name} (Value: ${formatForDisplay('Value', saleValue)} RP)</h4>`;
        recommendationsHtml += `<p>Target: Receive a bundle worth up to ${formatForDisplay('Value', saleValue / OPPONENT_SELL_ADVANTAGE)} RP.</p>`;
        ...
        sortedGroups.forEach(group => {
            ...
            recommendationsHtml += `<p style="margin-top: 10px;"><strong>Offer (Per Trade):</strong> 1x Mega ${refPet.name}</p>`;
            recommendationsHtml += `<p style="font-weight: bold; margin-top: 5px;">Receive Options (Count: ${group.count}):</p>`;
            group.bundles.forEach(bundle => {
                recommendationsHtml += `<p style="margin-left: 20px; margin-bottom: 5px; font-size: 0.95em;">
                    - **${bundle.count}x ${bundle.petName}** (Bundle Value: ${formatForDisplay('Value', bundle.bundleValue)} RP)
    ...
    if (neonSellBundles.length === 0 && megaSellBundles.length === 0) {
        recommendationsHtml += `<p>No highly profitable bundles (where the future profit is at least 10% of the aged ${refPet.name} value) could be found that fit within the 12-pet limit.</p>`;
    }
    recommendationsHtml += '</div>';
    
    // Always show inventory controls and header, but only show recommendations if tradesGenerated > 0
    if (tradesGenerated === 0) {
        return html + '<p>Trade data is unavailable for this pet (missing Neon/Mega values) or its value is too low to generate practical trades within the 12-pet limit.</p>';
    } else {
        return html + recommendationsHtml;
    }
}
export function getInventory(petName) {
    try {
        const inv = JSON.parse(localStorage.getItem('petInventory') || '{}');
        return inv[petName] || { regular: 0, neon: 0, mega: 0 };
    } catch { return { regular: 0, neon: 0, mega: 0 }; }
}
export function setInventory(petName, regular, neon, mega) {
    let inv = {};
    try { inv = JSON.parse(localStorage.getItem('petInventory') || '{}'); } catch {}
    inv[petName] = { regular, neon, mega };
    localStorage.setItem('petInventory', JSON.stringify(inv));
}
    html += `<p style="font-size: 1.1em; font-weight: bold; margin-bottom: 20px;">Current Value: ${formatForDisplay('Regular Value', refPet['Regular Value'])} RP</p>`;
